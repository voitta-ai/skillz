...
| [claude-code-settings-spec-from-binary](./skills/claude-code-settings-spec-from-binary/SKILL.md) | skill | Claude, Codex | Read an undocumented CC settings block out of the bun-compiled binary: `strings` to a file first, then prompt text for the slots and validator bodies for the limits |
| [`claude-code-settings-spec-from-binary` plugin](./plugins/claude-code-settings-spec-from-binary/) | plugin | Claude, Codex | Single-skill plugin: claude-code-settings-spec-from-binary |
...
| [vendor-console-form-silent-noop](./skills/vendor-console-form-silent-noop/SKILL.md) | skill | Claude, Codex | A vendor console's confirm button silently does nothing and the required consent checkbox clears itself on every submit. Two causes that imitate each other: a framework-controlled input that ignores synthetic clicks and needs a real `Space` keypress, and a server error the console renders nowhere. Tell them apart in one read-only step, read the swallowed response body, and check whether an earlier attempt half-succeeded before retrying something that costs money |
| [`vendor-console-form-silent-noop` plugin](./plugins/vendor-console-form-silent-noop/) | plugin | Claude, Codex | Single-skill plugin: vendor-console-form-silent-noop |
...
| [reverse-engineer-react-native-hermes-app](./skills/reverse-engineer-react-native-hermes-app/SKILL.md) | skill | Claude, Codex | Reverse-engineer a React Native Hermes-bytecode Android app off a stock non-rooted device: hermes-dec on unsupported bytecode versions, string-table flow tracing, the user-CA TLS-interception wall, and no-root BLE capture via the HCI snoop log |
| [`reverse-engineer-react-native-hermes-app` plugin](./plugins/reverse-engineer-react-native-hermes-app/) | plugin | Claude, Codex | Single-skill plugin: reverse-engineer-react-native-hermes-app |
...
| [homebrew-cask-adopt-app-management](./skills/homebrew-cask-adopt-app-management/SKILL.md) | skill | Claude, Codex | Adopt a hand-installed macOS app into a Homebrew cask: `--adopt` runs `sudo chgrp` over the bundle, which macOS App Management (TCC) blocks for any terminal lacking the grant, so it fails `Operation not permitted` after sudo already took the password and root does not help. Covers the grant-and-relaunch, why `sudo brew` is refused, the no-adopt alternative that keeps user data, and detaching the leftover DMG behind `hdiutil: attach failed - Resource busy` |
...
| [`homebrew-cask-adopt-app-management` plugin](./plugins/homebrew-cask-adopt-app-management/) | plugin | Claude, Codex | Single-skill plugin: homebrew-cask-adopt-app-management |
...
| [android-mic-capture-tears-down-a2dp](./skills/android-mic-capture-tears-down-a2dp/SKILL.md) | skill | Claude, Codex | Microphone capture opens a hands-free link and a car drops the music stream; diagnose from dumpsys and appops, then pin capture to the built-in mic |
